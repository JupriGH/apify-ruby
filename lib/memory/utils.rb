...
module Apify
	...
	module MemoryStorage::Utils ### FileUtils
		...
		def self._update_request_queue_item request_id:, request:, entity_directory:, persist_storage: nil
			# Skip writing files to the disk if the client has the option set to false
			return if !persist_storage 
			...
			file_path = File.join(entity_directory, "#{request_id}.json")				
			File.write file_path, request.to_json, encoding: 'utf-8'
		end
=begin
